...
**Worker:** worker1 **Goal:** a buildable `muse-code-acp` package whose ACP server completes a full prompt turn (streamed text + end_turn + cancel) against `muse exec --provider echo --json`, proving the process-per-turn architecture end to end before any real-model work. **Status:** todo

## Tasks (in order)

| id   | title                                                       | est | depends_on |
| ---- | ----------------------------------------------------------- | --- | ---------- |
| t001 | Scaffold repo: package, tooling, CI transplant              | 45m | —          |
| t002 | Transplant vendor-neutral shell (utils, index, logging)     | 45m | t001       |
